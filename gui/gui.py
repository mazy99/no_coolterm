...
class ModbusTerminal(QMainWindow):
    def __init__(self):
        ...
        self.syncing = (
            False  # синхронизация UI с пакетом (чтобы не было циклических обновлений)
        )
        self.current_packet = [0] * 8  # текущий формируемый пакет (8 байт)

        self.setWindowTitle("Modbus Терминал")
        self.setWindowIcon(QIcon(get_resource_path("gui/icon/icon.ico")))
    ...
    def setup_ui(self):
        ...
        self.status_indicator.setStyleSheet(
            "border-radius: 6px; background-color: #9CA3AF;"
        )
        ...
        default_led_style = (
            "border-radius: 6px; background-color: #9CA3AF; border: 1px solid #6B7280;"
        )
        # Индикатор TX
            ...
            if dev_id in DEVICE_MAPS and 0 in DEVICE_MAPS[dev_id]:
                ...
                if "Состояние " in param_text:
                    try:
                        ...
                        dev_name = (
                            param_text.split("Состояние ")[1].split("(")[0].strip()
                        )
        ...
        hex_validator_2 = QRegularExpressionValidator(
            QRegularExpression("[0-9A-Fa-f]{1,2}")
        )
        for i in range(8):
            byte_widget = QWidget()
        ...
        self.connect_btn.clicked.connect(
            lambda: [self.animate_button(self.connect_btn), self.on_connect()]
        )
        self.disconnect_btn.clicked.connect(
            lambda: [self.animate_button(self.disconnect_btn), self.on_disconnect()]
        )
        self.send_btn.clicked.connect(
            lambda: [self.animate_button(self.send_btn), self.on_send()]
        )
    ...
    def flash_tx(self):
        self.tx_indicator.setStyleSheet(
            "border-radius: 6px; background-color: #10B981; border: 1px solid #047857;"
        )
    ...
    def update_from_ui(self):
        ...
        try:
            ...
            if current_item_data == "custom":
                addr = self.current_packet[
                    0
                ]  # Если кастомный, берем строго из правой панели
            else:
            ...
            if cmd == 0x03:
                ...
                if not self.write_data.text().strip():
                    val_or_count = (self.current_packet[4] << 8) | self.current_packet[
                        5
                    ]
                else:
            ...
            hex_packet_str = self.modbus_engine.preview_request(
                addr, cmd, start, val_or_count
            )
            hex_bytes = (
                hex_packet_str.split()
            )  # Получаем список ['11', '03', '00', '08', ...]
    ...
    def on_connect(self):
        ...
        if self.modbus_engine.connect(config):
            ...
            self.status_indicator.setStyleSheet(
                "border-radius: 6px; background-color: #10B981;"
            )
    ...
    def format_registers_to_ascii_table(
        self, slave_id: int, start_address: int, registers: list
    ) -> str:
        ...
        h_addr, h_name, h_phys, h_scale, h_hex = (
            "Адрес",
            "Параметр",
            "Физическое знач",
            "Масштаб",
        ...
        lines.append(
            f"| {h_addr:<{w_addr}} | {h_name:<{w_name}} | {h_phys:<{w_phys}} | {h_scale:<{w_scale}} | {h_hex:<{w_hex}} |"
        )
        lines.append(sep)
        ...
        for r in rows:
            lines.append(
                f"| {r[0]:<{w_addr}} | {r[1]:<{w_name}} | {r[2]:>{w_phys}} | {r[3]:>{w_scale}} | {r[4]:<{w_hex}} |"
            )
    ...
    def on_send(self):
        ...
        if (
            self.device_addr.currentIndex() == -1
            or self.device_addr.currentData() == "custom"
        ):
        ...
        if self.cmd_combo.currentIndex() == 0:  # READ
            ...
            count = self.get_int(self.read_count.text(), 8)
            self.log(
                f"Чтение: {self.modbus_engine.preview_request(slave_id=slave_id, function_code=3, address=address, value_or_count=count)}"
            )

            self.flash_tx()

            try:
                result = self.modbus_engine.read_holding_registers(
                ...
                if registers:
                    ...
                    ascii_table = self.format_registers_to_ascii_table(
                        slave_id, address, registers
                    )
                    if ascii_table:
                        self.log_text.append(
                            f"<pre style='color: #000000;'>{ascii_table}</pre>"
                        )
            ...
            self.log(
                f"Запись: {self.modbus_engine.preview_request(slave_id=slave_id, function_code=6, address=address, value_or_count=value)}"
            )

            self.flash_tx()
            try:
                response = self.modbus_engine.write_single_register(
    ...
    def apply_styles(self):
        if not self.dark_mode:
            ...
            self.setStyleSheet(load_stylesheet("gui/styles/style_light_theme.qss"))
        else:
            self.theme_btn.setText("☀️")
            self.setStyleSheet(load_stylesheet("gui/styles/style_dark_theme.qss"))
